...
export default function DashboardAnalytics() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">{t('dash_analytics_title')}</h1>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="card p-5">
          <h2 className="text-xl font-bold mb-4">{t('dash_filter_crop')}</h2>
          <BarChart
            emptyLabel={t('dash_no_reports')}
      ...
      <section className="card p-5">
        <h2 className="text-xl font-bold mb-4">{t('dash_over_time')}</h2>

        {trends.length === 0 ? (
          <p className="text-slate-700">{t('dash_no_reports')}</p>
        ) : (
          <>
            ...
            <div className="flex items-end gap-1 h-40" aria-hidden="true">
              {trends.map((point) => (
                <div
                  key={point.date}
                  className="flex-1 bg-brand-700 rounded-t min-h-[2px]"
            ...
            <details className="mt-4">
              ...
              <ul className="mt-2 text-sm space-y-1">
                {trends.map((point) => (
                  <li key={point.date} className="flex justify-between border-b border-slate-100 py-1">
                    <span>{point.date}</span>
                    <span className="tabular-nums font-semibold">{point.count}</span>
                  </li>
                ))}
              </ul>
            </details>
